...
async function seedLedgerOpeningEntries() {
  console.log("=== SEEDING LEDGER OPENING ENTRIES FOR SEEDED DATA ===");
  ...
  // 1. Delete any existing opening balance ledger entries to make idempotent
  await prisma.cashLedgerEntry.deleteMany({
    where: {
      particular: { contains: "Opening Balance" },
    },
  });

  // 2. Customers Opening Entries
  const customers = await prisma.customer.findMany({
    where: { branchId: maharashtraBranch.id, deletedAt: null },
  });

  console.log(`Found ${customers.length} seeded customer records under Maharashtra branch.`);
  ...
  for (let i = 0; i < customers.length; i++) {
    ...
    if (bal === 0) continue; // Skip zero balance parties or include if non-zero
  ...
  // 3. Vendors Opening Entries
  const vendors = await prisma.vendor.findMany({
    where: { branchId: maharashtraBranch.id, deletedAt: null },
  });

  console.log(`Found ${vendors.length} seeded vendor records under Maharashtra branch.`);
...
seedLedgerOpeningEntries()
  .catch((err) => {
    console.error("Seeding ledger opening entries failed:", err);
    process.exit(1);
  })
  .finally(async () => {
