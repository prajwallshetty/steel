...
export function PWAInstallButton({ className, showLabel = true }: PWAInstallButtonProps) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const handleAppInstalled = () => {
        ...
        toast.success("LSC Alloys ERP app installed successfully!");
      };
  ...
  const handleInstallClick = async () => {
    if (isInstalled) {
      toast.info("App is already installed and running on your device.");
      return;
    }
  ...
  if (isInstalled) {
    return (
      <div className="flex items-center gap-2 px-3 py-2 text-xs font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 rounded-lg border border-emerald-200 dark:border-emerald-800/50 select-none">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="relative w-full max-w-sm bg-card text-card-foreground border rounded-2xl p-6 shadow-2xl space-y-5 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-bold">Install LSC Alloys ERP</h3>
                <p className="text-xs text-muted-foreground">Add to iPhone / iPad Home Screen</p>
              </div>
            </div>
